...
CLANG_COMPLETE_CONTENTS=$(cat ${SOURCE_PATH}/.clang_complete)

add_to_index() {
    INDEX_FILE=`echo ${1}.i.gz | tr "/" "%"`
    echo clic_add index.db $INDEX_FILE ${CLANG_COMPLETE_CONTENTS} $1
    clic_add index.db $INDEX_FILE  ${CLANG_COMPLETE_CONTENTS} $1
}
